...
- **Map Rendering**: SVG in `partials/azeroth-map.svg` (Kalimdor, Eastern Kingdoms, the WoW: Forever zones and the Maelstrom). `server.py` inlines it into `index.html` at `<!-- azeroth-map.svg -->` so CSS and click handlers can reach the zones. The paths are generated by `tools/generate_map.py`: edit coastlines, zone seeds or district labels there and re-run `uv run --no-project --with shapely python tools/generate_map.py`. Never hand-edit the path data.
...
4. **Codebase Cleanliness & Simplicity**:
   - Keep the UI build-free: plain HTML pages, plain CSS files and classic `<script>` files under `static/` (no modules, so inline `onclick` handlers keep working; load `election-map.js` before `app.js`). Do not introduce Webpack, Vite, React, or TypeScript build steps unless explicitly requested by the user.
